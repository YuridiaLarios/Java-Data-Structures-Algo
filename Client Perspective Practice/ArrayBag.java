...
public final class ArrayBag<T extends Comparable<? super T>> implements BagInterface<T> {
	 ...
	 * and sends it to the second constructor */
	public ArrayBag() {
...
/*	
 ...
 This method returns a new bag of items that are less than the item passed as a parameter. 
  The method does not alter the current bag.
	Hint: recall that you can directly access the instance data variables, including the array.
	...
	*/
	...
	// and returns the object that is replaced (removed)
		public T replace(T givenObject) {
			T removedObject = this.remove();
			add(givenObject);
		...
		// This method removes all occurrences of a specific entry (send as a parameter) from a bag
		public void removeEvery(T givenEntry){
			while (Arrays.asList(bag).contains(givenEntry)) {
					this.remove(givenEntry);
			}		
		}
